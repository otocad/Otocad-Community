...
public class CadCanvas : Control, ICadCommandHost
{
    ...
    void ICadCommandHost.AddEntity(Entity entity)
    {
        ...
        // 默认归到 "0" 图层 (命令可后续 AssignEntityToLayer 改)
        AssignEntityToLayer(entity, GetOrCreateLayer("0"));
    ...
    /// </summary>
    public void RefreshAutoDimensionsFor(Entity source)
    {
        if (source is null) return;
        // 找出 Owner = source 的全部自动标注实体 (尺寸 + 镀膜标记等), 一并刷新。
        // 不能只过滤 DimensionBase, 否则镀膜标记每次刷新都不被清除 → 累积重复。
        var toRemove = _entities
            .Where(e => ReferenceEquals(e.Owner, source))
            .ToList();
        if (toRemove.Count == 0) return;
        foreach (var d in toRemove) _entities.Remove(d);
        // 重新生成
        var defaultLayer = GetOrCreateLayer("0");
        foreach (var newDim in OtoCAD.Avalonia.Commands.AutoDimensionLensCmd.Build(source))
        {
            newDim.Owner = source;
    ...
    /// <summary>Phase 1F+: 在选中集合的实体上查 grip 命中 (优先 grip 而非实体).</summary>
    ...
    public void LoadTestScene()
    {
        ...
        // ISO 10110 竖版单透镜图纸样例 (平凸 PCX-001) — 由可复用生成器构建, 锁定图框层
        var frameLayer = GetOrCreateLayer("图框");
        frameLayer.IsLocked = true;
        foreach (var ent in OtoCAD.Avalonia.Commands.IsoSingleLensSheet.BuildSample())
        {
            _entities.Add(ent);
    ...
    private void RenderSceneCentered(SKCanvas skCanvas, int width, int height)
    {
        ...
        {
            gd.CurrentSkColor = ResolveEntitySkColor(ent);
    ...
    /// </summary>
    private static SKColor ResolveEntitySkColor(Entity e)
    {
        var c = e.color;
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        if (e.Key == Key.Escape)
        {
            ...
            else if (_selectedEntities.Count > 0) { ClearSelection(); e.Handled = true; }
        }
        // Phase 1A: Del 删除选中实体
    ...
    protected override void OnPointerMoved(PointerEventArgs e)
    {
        ...
        if (_gripEntity is not null && _gripPoint is not null)
        {
            ...
            if (!stuck) _gripEntity.SetGripPointAt(_gripIndex, _gripPoint, target);
