...
public partial class DrawingConventionDialog : Window
{
    ...
    private readonly ComboBox _baseCombo, _sqCombo, _faCombo, _roCombo, _ldCombo, _unitsCombo, _dimCombo, _styleCombo, _frameCombo;
    ...
    public DrawingConventionDialog(DrawingConvention initial)
    {
        ...
        _frameCombo = this.FindControl<ComboBox>("FrameCombo")!;
        ...
        // 图框模板: 内置键 + Config/Frames 数据驱动定义名 (热加载); 当前惯例的键不在列表也保留可选
        var frameKeys = FrameTemplateService.AllKeys().ToList();
        if (!string.IsNullOrWhiteSpace(initial.FrameTemplateKey) && !frameKeys.Contains(initial.FrameTemplateKey))
            frameKeys.Add(initial.FrameTemplateKey);
        _frameCombo.ItemsSource = frameKeys;
        ...
        foreach (var combo in new[] { _sqCombo, _faCombo, _roCombo, _ldCombo, _unitsCombo, _dimCombo, _styleCombo, _frameCombo })
    ...
    private void LoadFrom(DrawingConvention c)
    {
        ...
        if (_frameCombo.ItemsSource is System.Collections.Generic.List<string> keys
            && !string.IsNullOrWhiteSpace(c.FrameTemplateKey) && !keys.Contains(c.FrameTemplateKey))
        {
            keys.Add(c.FrameTemplateKey);
            _frameCombo.ItemsSource = keys.ToList();   // 换实例触发刷新
        }
        _frameCombo.SelectedItem = string.IsNullOrWhiteSpace(c.FrameTemplateKey) ? "gb-lens" : c.FrameTemplateKey;
    ...
    private DrawingConvention Gather()
    {
        return new DrawingConvention
        {
            ...
            FrameTemplateKey = _frameCombo.SelectedItem as string ?? "gb-lens",
        };
    }
