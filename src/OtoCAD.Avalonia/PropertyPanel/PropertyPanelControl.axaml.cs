...
public partial class PropertyPanelControl : UserControl
{
    ...
    public void SetFrameCell(string zone, string label, string currentText, Action<string> onSet,
        ...
        string? cellKey = null, Action<string>? onSetKey = null)
    {
        ...
        // 自定义属性"键"编辑 (供清单/AI 按键引用; 失焦才写回, 改键代价大不逐字符)。
        if (cellKey is not null && onSetKey is not null)
        {
            var kgrid = new Grid { ColumnDefinitions = new ColumnDefinitions("70,*") };
            kgrid.Children.Add(new TextBlock { Text = "键 key", VerticalAlignment = VerticalAlignment.Center, FontSize = 11 });
            var kbox = new TextBox { Text = cellKey, Margin = new Thickness(0, 2), FontSize = 11 };
            Grid.SetColumn(kbox, 1);
            kbox.LostFocus += (_, _) => onSetKey(kbox.Text ?? "");
            kgrid.Children.Add(kbox);
            _contentPanel.Children.Add(kgrid);
        }
