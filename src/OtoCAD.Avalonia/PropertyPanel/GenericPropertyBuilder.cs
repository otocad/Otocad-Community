...
public static class GenericPropertyBuilder
{
    ...
    private static readonly Dictionary<string, string> ZhLabels = new(StringComparer.Ordinal)
    {
        ...
        ["ShowRc"] = "标 Rc 胶合面",
        ["ShowR3"] = "标 R3 半径",
        ["ShowThickness"] = "标 中心厚度 d",
        ["ShowDiameter"] = "标 通光口径 Ø",
        ["ShowCoating"] = "标 AR 镀膜",
        // 材料
        ["MaterialName"] = "材料",
    ...
    /// <summary>属性名 → 悬停提示 (引导用户该标什么 / 对应国标依据). 未列出的无提示.</summary>
    private static readonly Dictionary<string, string> ZhTips = new(StringComparer.Ordinal)
    {
        ["ShowOpticalAxis"] = "光轴双点画线 (GB/T 13323-2009 §2.1) — 光学件区别于普通几何件的标志。",
        ["ShowR1"] = "前表面曲率半径 R1。ISO 10110-1 符号约定: R>0 曲率中心在面右侧。",
        ["ShowR2"] = "后表面曲率半径 R2。",
        ["ShowRc"] = "胶合面曲率半径 Rc — 两片公共面, 决定消色差匹配。",
        ["ShowR3"] = "后表面曲率半径 R3。",
        ["ShowThickness"] = "中心厚度 d — 光学零件图必标项 (GB/T 13323-2009 §3)。",
        ["ShowDiameter"] = "通光口径 Ø (净口径) — 表面在此口径内有效。",
        ["ShowCoating"] = "增透膜 AR 符号 (GB/T 13323-2009)。镀膜不在图上常被漏做。",
    };
    ...
    public static IReadOnlyList<Control> Build(Entity entity, Action onChanged)
    {
        ...
        // 标注标准 — 实体级覆盖 (混标): 每条标注可独立选标准, 改它即重置自身 style.
        if (entity is DimensionBase dim)
            sections.Add(MakeStandardSection(dim, onChanged));
    ...
    /// <summary>标注标准下拉 — 选中即把该标注的 style 重置为对应标准 (实体级覆盖/混标, D9).</summary>
    private static Control MakeStandardSection(DimensionBase dim, Action onChanged)
    {
        var keys = OtoCAD.Avalonia.Services.DimensionStandardService.AllKeys;
        var curKey = OtoCAD.Avalonia.Services.DimensionStandardService.KeyFromStyle(dim.style);

        var grid = new Grid { ColumnDefinitions = new ColumnDefinitions("70,*") };
        grid.Children.Add(new TextBlock { Text = "标准", VerticalAlignment = VerticalAlignment.Center, FontSize = 11 });
        var combo = new ComboBox { Margin = new Thickness(0, 2), FontSize = 11 };
        foreach (var k in keys)
            combo.Items.Add(OtoCAD.Avalonia.Services.DimensionStandardService.GetDisplayName(k));
        var curIdx = Array.IndexOf(keys, curKey);
        ...
        combo.SelectionChanged += (_, _) =>
        {
            ...
            var newKey = keys[i];
            if (OtoCAD.Avalonia.Services.DimensionStandardService.KeyFromStyle(dim.style) == newKey) return;
            dim.style = OtoCAD.Avalonia.Services.DimensionStandardService.CreateStyle(newKey);
            onChanged();
        };
        grid.Children.Add(combo);

        return MakeSection("标注标准", grid,
            MakeReadonlyRow("说明", "可独立于其他标注 (混标)"));
    }
    ...
    private static Control MakeReflectionEditor(Entity e, Action onChanged)
    {
        ...
        var showRows = new List<Control>();   // Show* 复选框单独成组 ("标注显示"), 作为引导式标注清单
        var t = e.GetType();
        ...
        foreach (var prop in t.GetProperties(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            try
            {
                ...
                {
                    var boolRow = MakeBoolRow(Label(prop.Name), (bool)prop.GetValue(e)!, v => { prop.SetValue(e, v); onChanged(); });
                    if (ZhTips.TryGetValue(prop.Name, out var tip)) ToolTip.SetTip(boolRow, tip);
                    if (prop.Name.StartsWith("Show", StringComparison.Ordinal)) showRows.Add(boolRow);
                    else rows.Add(boolRow);
                }
        ...
        // 几何/材料 + 独立的"标注显示"清单 (引导用户勾选要标注的项)
        var panel = new StackPanel { Spacing = 4 };
        panel.Children.Add(MakeSection("属性", rows.ToArray()));
        panel.Children.Add(MakeSection("标注显示 · 勾选要标的项", showRows.ToArray()));
        return panel;
    }
}
