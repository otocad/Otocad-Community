...
public static class AutoDimensionLensCmd
{
    ...
    private static void ApplyToSingle(OpticalLens lens, Action<Entity> add, DimensionStyle dimStyle, AutoDimLayoutPolicy layout, bool includeRadial = true)
    {
    ...
    private static void ApplyToCemented(CementedLens lens, Action<Entity> add, DimensionStyle dimStyle, AutoDimLayoutPolicy layout, bool includeRadial = true)
    {
    ...
    /// internal: IsoSingleLensSheet 复用 (双标注原则: 表格 7/ 行 + 正文符号都要)。
    /// </summary>
    internal static void EmitCoatingMark(
        double apexX, double axisY, double halfD, double r, bool glassOnPlusX, Action<Entity> add)
    {
        ...
        mark.AttachState = new lcdb.Annotation.SurfaceAttachState(new Vector2(apexX, axisY), r, glassOnPlusX);  // 拖动沿面滑
        add(mark);
    }
