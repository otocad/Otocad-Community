...
namespace lcdb.Tests
{
    ...
    public class DrawingConventionServiceTests
    {
        ...
        [TestMethod]
        public void Clone_copies_frame_template_key()
        {
            // 图框轴 (三轴之一) 必须随派生/另存为一起走, 否则自定义图框选择在"另存为"后悄悄回落 gb-lens
            var c = new DrawingConvention { Name = "x", FrameTemplateKey = "GB-my-1", DrawingStandardName = "ISO" };
            var d = c.Clone();
            Assert.AreEqual("GB-my-1", d.FrameTemplateKey);
            Assert.AreEqual("ISO", d.DrawingStandardName);
        }
