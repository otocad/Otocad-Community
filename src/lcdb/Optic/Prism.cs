...
public sealed class Prism : Entity
{
    ...
    /// <summary>玻璃剖面底色 (null = 不填充). 与 OpticalLens.FillColor 同约定, 出图时由装配器设。</summary>
    ...
    /// <summary>剖面多边形顶点 (直角 → 底边右端 → 竖边上端, 含 Rotation). 供玻璃填充用.</summary>
    private List<Vector2> CrossSectionPolygon()
    {
        double halfW = Width * 0.5, halfH = Height * 0.5;
        var pts = new List<Vector2>
        {
            new(Position.X - halfW, Position.Y - halfH),
            new(Position.X + halfW, Position.Y - halfH),
            new(Position.X - halfW, Position.Y + halfH),
        };
        if (Rotation != 0)
        {
            ...
            {
                var d = pts[i] - Position;
                pts[i] = Position + new Vector2(d.X * c - d.Y * s, d.X * s + d.Y * c);
            }
        }
        return pts;
    }
    ...
    public override void Draw(IGraphicsDraw gd)
    {
        ...
        // 玻璃剖面底色: 轮廓之前先画 (轮廓盖在上面), 与 OpticalLens.Draw 同约定
        if (FillColor.HasValue)
        {
            var prev = gd.CurrentColor;
            gd.CurrentColor = FillColor.Value.ToDrawingColor();
            gd.DrawFilledPolygon(CrossSectionPolygon());
