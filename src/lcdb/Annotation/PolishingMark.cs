...
namespace lcdb.Annotation
{
    ...
    public class PolishingMark : Entity, ISurfaceAttachable
    {
        ...
        protected void Generate()
        {
            ...
            // 三角形顶点在下 (apexY)、底边在上, 半宽随高度线性从 0 增到 half。
            // 等级线须落在三角形内: 用该高度处的三角半宽插值, 再内缩 0.85 不顶到斜边。
            double apexY = Center.Y - Size * 2.0 / 3.0;
            for (int i = 0; i < lineCount; i++)
            {
                double y = Center.Y - Size / 3.0 + i * Size / 6.0;
                double w = 0.5 * (y - apexY) * 0.85;   // = half*(y-apexY)/(baseY-apexY)*0.85
