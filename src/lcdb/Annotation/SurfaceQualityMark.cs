...
namespace lcdb.Annotation
{
    ...
    public class SurfaceQualityMark : Entity, IOpticalMark, ISurfaceAttachable
    {
        ...
        /// <summary>
        /// 长划痕条数 Nl —— ISO 10110-7:2017 §4.2.1.4 长划痕项 <c>; L Nl×Al</c>
        /// (长划痕 = 长度 &gt;2mm 的划痕; 普通划痕/麻点已并入 N×A 总数, 标准无单独项)。
        /// </summary>
        public int ScratchCount { get; set; } = 0;

        /// <summary>
        /// 长划痕最大宽度 Al (mm) —— ISO 10110-7:2017 §4.2.1.4, 宽度取 R5 系列
        /// (0.1; 0.063; 0.04; 0.025; 0.016; 0.01; 0.006; 0.004)。
        /// </summary>
        ...
        /// <param name="scratchCount">长划痕条数 Nl (ISO §4.2.1.4 L 项)</param>
        /// <param name="scratchWidth">长划痕最大宽度 Al mm (ISO §4.2.1.4 L 项)</param>
        public SurfaceQualityMark(Vector2 position, int defectCount, double defectSize,
            int scratchCount = 0, double scratchWidth = 0.0)
        {
            Position = position;
            QualityStandard = SurfaceQualityStandard.ISO_10110_7;
        ...
        /// <summary>
        /// 更新 ISO 10110-7:2017 尺寸法格式串。
        /// 基本式 §4.2.1.1: <c>5/Ng×Ag</c> (Ng=疵病数, Ag=等级=最大允许面积的平方根 mm)。
        /// 长划痕项 §4.2.1.4: <c>; L Nl×Al</c> (Nl=长划痕条数, Al=最大宽度 mm)，分号分隔、带字母代号 L。
        /// 标准的每个补充段都必须带字母代号 (W 宽度 / C 镀膜 / L 长划痕 / E 崩边)，
        /// 不存在无代号的裸 <c>; N×A</c> 段。镀膜 C / 宽度 W / 崩边 E 项本类暂未建模 (见合规文档已知缺口)。
        /// </summary>
        private void UpdateISOMarkText()
        {
            var sb = new StringBuilder();
            sb.Append("5/");                                  // §4.1 个体表面/元件代号
            sb.Append(FormatNxA(DefectCount, DefectSize));    // §4.2.1.1 基本式 Ng×Ag

            // §4.2.1.4 长划痕: ; L Nl×Al (字母代号 L 不可省)
            if (ScratchCount > 0 && ScratchWidth > 0)
            {
                sb.Append("; L");
                sb.Append(FormatNxA(ScratchCount, ScratchWidth));
            }
        ...
        /// <summary>
        /// 渲染 <c>N×A</c> 子串: N 为整数, A 为等级值 (最多 3 位小数, 兼容 R5 系列最小 0.004,
        /// 去尾零)，统一用不变区域性 (小数点)，避免随系统区域出现逗号。
        /// </summary>
        private static string FormatNxA(int n, double a)
        {
            string grade = a.ToString("0.###", CultureInfo.InvariantCulture);
            return string.Concat(n.ToString(CultureInfo.InvariantCulture), "×", grade);
        }
...
#region IOpticalMark 接口实现
        ...
        private bool ValidateISO()
        {
            ...
            // 长划痕项: 有条数必须有宽度 (ISO §4.2.1.4 的 L Nl×Al)
            if (ScratchCount > 0 && ScratchWidth <= 0)
                return false;
