...
namespace lcdb.Annotation
{
    ...
    public class ISO10110_14Mark : Entity, IOpticalMark
    {
        ...
        public string MarkText { get; set; } = "14/0.25λ RMS";
        ...
        private void UpdateMarkText()
        {
            ...
            MarkText = $"14/{WavefrontError:F2}λ {typeStr}";
        }
        ...
        private void GenerateMainSymbol()
        {
            ...
            // ISO 10110-14 严格格式 "13/A"  (A=RMS 波数)
            string codeText = WavefrontError > 0
                ? $"13/{WavefrontError:G3}"
                : "13/—";
        ...
        {
            // RMS误差颜色编码
            if (WavefrontError <= 0.1)
                return lcdb.Colors.Color.FromColor(System.Drawing.Color.Red);      // 高精度 - 红色
            else if (WavefrontError <= 0.25)
                return lcdb.Colors.Color.FromColor(System.Drawing.Color.Orange);   // 精密级 - 橙色
            else if (WavefrontError <= 0.5)
                return lcdb.Colors.Color.FromColor(System.Drawing.Color.Green);    // 标准级 - 绿色
            else
                return lcdb.Colors.Color.FromColor(System.Drawing.Color.Blue);     // 一般级 - 蓝色
        }
        #endregion
