...
namespace lcdb.Annotation
{
    ...
    public class FocalPointMark : Entity, IOpticalMark
    {
        ...
        public override Bounding bounding
        {
            get
            {
                double totalSize = SymbolSize * Scale * 2;
                
                if (ShowText)
                {
                    totalSize = Math.Max(totalSize, Math.Abs(TextOffset.X) + 60);
                    totalSize = Math.Max(totalSize, Math.Abs(TextOffset.Y) + 30);
                }

                if (ShowBeam)
                {
                    double beamSize = FocalLength * Math.Tan(BeamConeAngle * Math.PI / 180) * 2;
                    totalSize = Math.Max(totalSize, beamSize);
                }

                return new Bounding(Position, totalSize, totalSize);
            }
        }
        ...
        private void GenerateCrossSymbol(double size, lcdb.Colors.Color color)
        {
            ...
            var horizontalLine = new Line(
                new Vector2(Position.X - halfSize, Position.Y),
                new Vector2(Position.X + halfSize, Position.Y))
            { color = color };

            var verticalLine = new Line(
                new Vector2(Position.X, Position.Y - halfSize),
                new Vector2(Position.X, Position.Y + halfSize))
            { color = color };
        ...
        private void GenerateSquareSymbol(double size, lcdb.Colors.Color color)
        {
            ...
            var p1 = new Vector2(Position.X - halfSize, Position.Y - halfSize);
            var p2 = new Vector2(Position.X + halfSize, Position.Y - halfSize);
            var p3 = new Vector2(Position.X + halfSize, Position.Y + halfSize);
            var p4 = new Vector2(Position.X - halfSize, Position.Y + halfSize);
        ...
        private void GenerateDiamondSymbol(double size, lcdb.Colors.Color color)
        {
            ...
            var p1 = new Vector2(Position.X, Position.Y + halfSize);      // 上
            var p2 = new Vector2(Position.X + halfSize, Position.Y);      // 右
            var p3 = new Vector2(Position.X, Position.Y - halfSize);      // 下
            var p4 = new Vector2(Position.X - halfSize, Position.Y);      // 左
            _markEntities.Add(new Line(p1, p2) { color = color });
            _markEntities.Add(new Line(p2, p3) { color = color });
            _markEntities.Add(new Line(p3, p4) { color = color });
            _markEntities.Add(new Line(p4, p1) { color = color });
        ...
        private void GenerateEllipticalMark(double size, lcdb.Colors.Color color)
        {
            ...
            // 水平椭圆
            var p1 = new Vector2(Position.X - halfHeight, Position.Y - halfWidth);
            var p2 = new Vector2(Position.X + halfHeight, Position.Y - halfWidth);
            var p3 = new Vector2(Position.X + halfHeight, Position.Y + halfWidth);
            var p4 = new Vector2(Position.X - halfHeight, Position.Y + halfWidth);
        ...
        private void GenerateBeam()
        {
            ...
            // 光束的两条边界线
            var topPoint = Position + new Vector2(
                beamLength * Math.Cos(-halfAngle),
                beamLength * Math.Sin(-halfAngle));
                
            var bottomPoint = Position + new Vector2(
                beamLength * Math.Cos(halfAngle),
                beamLength * Math.Sin(halfAngle));
            ...
            if (NumericalAperture > 0)
            {
                ...
                naText.Position = new Vector3(Position.X + beamLength + 5, Position.Y, 0.0);
        ...
        private void GenerateText()
        {
            ...
            focalLengthText.Position = new Vector3(textPosition.X, textPosition.Y - 30 * Scale, 0.0);
            ...
            if (BeamQuality != 1.0)
            {
                ...
                beamQualityText.Position = new Vector3(textPosition.X, textPosition.Y - 45 * Scale, 0.0);
