...
{
    /// <summary>
    /// 光学表面结构(表面纹理)标记 —— GB/T 13323-2009 §附录C / ISO 10110-8 的 G/P 表示:
    /// 粗糙面 G(ground, 用 Rq), 抛光面 P(polished, P1–P4 / Rq / PSD)。无 slash 代号
    /// (ISO 10110-8 不用 `N/` 代号, 故本类默认 G/P 而非旧的凭空 `8/`)。
    /// 与机械粗糙度 <see cref="SurfaceRoughnessMark"/>(GB/T 131 Ra V 形, 通用)并存、各管各:
    /// 光学表面结构走本类的 G/P, 一般机械粗糙度走 Ra V 形。
    /// </summary>
    public class SurfaceTextureMark : Entity, IOpticalMark, ISurfaceAttachable
    {
        ...
        private void GenerateMainSymbol()
        {
            ...
            // G/P 模式: 主行直接画 G/P 代码(与 MarkText 一致); 否则 Ra 主行 + (可选 Rz/Rq) 副行。
            string mainLine;
            string subParts = "";
            if (UseGB13323Format)
            {
                mainLine = MarkText;   // 光学表面结构代码, 如 "P3/Rq0.002" / "G/Rq2"
            }
            else
            {
                mainLine = RaValue > 0 ? $"Ra {RaValue:G3}" : "Ra —";
