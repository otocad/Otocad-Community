...
namespace lcdb.Annotation
{
    ...
    public class SurfaceRoughnessMark : Entity, ISurfaceAttachable
    {
        ...
        protected void Generate()
        {
            ...
            double half = Size * 0.5;
            double barY = Center.Y + Size * 2.0 / 3.0;  // 长边顶 / 顶部水平线的 Y
            var apex   = R(new Vector2(Center.X,            Center.Y - Size * 2.0 / 3.0));
            var leftB  = R(new Vector2(Center.X - half * 0.7, Center.Y + Size / 3.0));  // 短边顶(开口, 低于长边)
            ...
            // GB/T 131-2006 完整图形符号: 长边顶端「标注横线」水平向右伸, 参数值(Ra/Rz)写在其上方。
            // 该横线是完整符号的固定组成部分, 基本(任意)/去除/不去除三种修饰符都有, 故无条件绘制。
            var barEnd = R(new Vector2(Center.X + Size * 1.8, barY));
            _markEntities.Add(new Line(rightB, barEnd) { color = color });

            // GB/T 131-2006 4.1.2 "去除材料": 封口横杠连接短(左)边顶 → 长(右)边同高点, 闭合成三角形。
            // 横杠与短边顶平齐 (Y = Center.Y + Size/3), 低于长边顶的标注横线 (barY);
            // 长边继续上伸到 rightB 承载标注横线。
            if (Removal == MaterialRemoval.Remove)
            {
                // leftB 即短边顶; 长边在该高度处 X = Center.X + half*0.75 (沿 apex→rightB 取 t=3/4)
                var barRight = R(new Vector2(Center.X + half * 0.75, Center.Y + Size / 3.0));
                _markEntities.Add(new Line(leftB, barRight) { color = color });
            }
            // GB/T 131-2006: "不去除材料" — 基本符号上加一内接圆 (绕 Center 旋转后圆心不变)。
            ...
            // MaterialRemoval.Any → 基本符号(仅 V + 标注横线), 表示任意加工方法

            if (ShowText)
            {
                // GB/T 131-2006 Figure 1 / 5.3 - 参数值 (Ra) 写在标注横线 *上方*, 且与该横线平行。
                // 文字随符号旋转到与标注横线同向 (规整到可读区间 (-π/2, π/2], 避免颠倒)。
                double raTextRot = Math.Atan2(Math.Sin(Rotation), Math.Cos(Rotation));
                if (raTextRot > Math.PI / 2.0) raTextRot -= Math.PI;
                else if (raTextRot <= -Math.PI / 2.0) raTextRot += Math.PI;
