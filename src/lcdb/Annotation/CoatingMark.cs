...
namespace lcdb.Annotation
{
    ...
    public class CoatingMark : Entity, ISurfaceAttachable
    {
        ...
        private void GenerateStandardMark()
        {
            ...
            {
                case CoatingType.HR:
                    // #10 内反射膜 ⊗ - 圆圈内X形(两条对角线交叉)
                    GenerateCircleWithX();
                    break;
                case CoatingType.OuterReflective:
                    // #11 外反射膜 ⊘ - 圆圈内单条斜杠
                    GenerateCircleWithSlash();
                    break;
                case CoatingType.BS:
                    // #12 分束膜 Y - Y形符号(无外圈)
                    GenerateYShape();
                ...
                case CoatingType.BBAR:
                    // #18 减反射膜 ⊕ - 圆圈内十字(与偏振膜相同)
                    GenerateCircleWithCross();
                    break;
                case CoatingType.PR:
                    // 部分反射膜 - 圆圈内半填充三角(非标准,保留兼容)
                    GenerateCircleWithHalfTriangle();
                    break;
                default:
                    // 其他/自定义 - 简单圆圈
                    GenerateOuterCircle();
        ...
        /// <summary>
        /// #10 内反射膜 ⊗ - 圆圈内X形(两条对角线交叉)
        /// GB/T 13323-2009 表1 序号10
        /// </summary>
        private void GenerateCircleWithX()
        {
            GenerateOuterCircle();

            double r = Size * 0.5;
            double d = r * 0.707; // √2/2 ≈ 0.707，使线端点接触圆圈

            // 左下到右上的对角线
            _markEntities.Add(new Line(
                R(new Vector2(Center.X - d, Center.Y - d)),
                R(new Vector2(Center.X + d, Center.Y + d))) { color = GetMarkColor() });
        ...
        /// <summary>
        /// #11 外反射膜 ⊘ - 圆圈内单条斜杠
        /// GB/T 13323-2009 表1 序号11
        /// </summary>
        private void GenerateCircleWithSlash()
        {
            GenerateOuterCircle();
        ...
        /// 注意: 分束膜不需要外圈
        /// </summary>
        private void GenerateYShape()
        {
            // 不调用GenerateOuterCircle() - 分束膜无外圈
        ...
        /// 注意: 导电膜不需要外圈
        /// </summary>
        private void GenerateTripleWave()
        {
            // 不调用GenerateOuterCircle() - 导电膜无外圈

            double r = Size * 0.5;
            double lineSpacing = Size * 0.25; // 三条线之间的间距
            double[] yOffsets = { -lineSpacing, 0, lineSpacing }; // 上中下三条线

            foreach (var yOff in yOffsets)
            {
                GenerateSingleWaveLine(Center.X - r, Center.X + r, Center.Y + yOff);
            }
        }
        ...
        /// <summary>
        /// 部分反射膜 - 圆圈内半填充三角（顶点接触圆圈）
        /// </summary>
        private void GenerateCircleWithHalfTriangle()
        {
            GenerateOuterCircle();

            double r = Size * 0.5;
            // 等边三角形内接于圆，顶点在圆上
            var p1 = R(new Vector2(Center.X, Center.Y + r)); // 顶点
            var p2 = R(new Vector2(Center.X - r * Math.Sqrt(3) / 2, Center.Y - r * 0.5)); // 左下
            var p3 = R(new Vector2(Center.X + r * Math.Sqrt(3) / 2, Center.Y - r * 0.5)); // 右下
            ...
            // 添加中线表示半填充（从顶点到底边中点）
            var mid23 = new Vector2((p2.X + p3.X) / 2, (p2.Y + p3.Y) / 2);
            _markEntities.Add(new Line(p1, mid23) { color = GetMarkColor() });
        }
        ...
        public override void TransformBy(Matrix3 transform)
        {
            ...
            Size = (transform * sizeVector).length;
            _markEntities.Clear();
        }
