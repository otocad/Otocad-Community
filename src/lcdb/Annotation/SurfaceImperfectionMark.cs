...
{
    /// <summary>
    /// 【已废弃 2026-06-04】已合并入 <see cref="SurfaceQualityMark"/>(同 ISO 10110-7 疵病, 代号 5/,
    /// 且支持 ISO/MIL 双标准)。已移出 Ribbon/命令;仅保留类供旧 .otocad 反序列化, 勿新增引用。
    ///
    ...
    public class SurfaceImperfectionMark : Entity, IOpticalMark, ISurfaceAttachable
    {
        ...
        /// 标记文本内容
        /// </summary>
        public string MarkText { get; set; } = "5/0.1x0.063;0.025";
        ...
        {
            // ISO 10110-7 表面疵病的指示代号是 "5/"(ISO 10110-10 代号表;非部件号 7)
            MarkText = $"5/{LinearDefectLength:F3}x{LinearDefectWidth:F3};{PointDefectDiameter:F3}";
        }
