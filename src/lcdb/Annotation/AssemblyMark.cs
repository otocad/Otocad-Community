...
namespace lcdb.Annotation
{
    ...
    public class AssemblyMark : Entity, IOpticalMark
    {
        ...
        private void GenerateAssemblyLabel(double halfHeight, lcdb.Colors.Color color)
        {
            var line1 = new Text
            {
                Value = $"{GetAssemblyTypeDescription()}: {AssemblySequence}",
...
#region IOpticalMark 接口实现
        ...
        public void Draw(Graphics g, float scale)
        {
            ...
            DrawAssemblySymbol(g, position, scale * (float)Scale);
        }

        /// <summary>
        /// 绘制装配符号
        /// </summary>
        private void DrawAssemblySymbol(Graphics g, PointF position, float scale)
        {
            ...
            using (var font = new Font("Arial", 6 * scale, System.Drawing.FontStyle.Bold))
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    octagonPoints[i] = new PointF(
                        position.X + radius * (float)Math.Cos(angle),
                        position.Y + radius * (float)Math.Sin(angle)
                    );
                }
                g.DrawPolygon(pen, octagonPoints);

                // 绘制装配符号（简化版
                DrawSimpleAssemblySymbol(g, pen, position, scale);

                // 绘制序号
                g.DrawString(AssemblySequence, font, brush, position.X, position.Y + radius * 0.7f,
                           new StringFormat { Alignment = StringAlignment.Center, LineAlignment = StringAlignment.Center });

                // 绘制状态指示器
                DrawStatusIndicator(g, position, radius, scale);
            }
        }
        ...
        /// 绘制简化的装配符号
        /// </summary>
        private void DrawSimpleAssemblySymbol(Graphics g, Pen pen, PointF position, float scale)
        {
            float symbolSize = 10 * scale;
            
            switch (AssemblyCategory)
            {
                case AssemblyType.Optical:
                    // 透镜装配符号
                    g.DrawEllipse(pen, position.X - symbolSize * 0.25f, position.Y - symbolSize * 0.1f,
                                 symbolSize * 0.2f, symbolSize * 0.2f);
                    g.DrawEllipse(pen, position.X + symbolSize * 0.05f, position.Y - symbolSize * 0.1f,
                                 symbolSize * 0.2f, symbolSize * 0.2f);
                    g.DrawLine(pen, position.X - symbolSize * 0.3f, position.Y,
                              position.X + symbolSize * 0.3f, position.Y);
                    break;
                    
                case AssemblyType.Mechanical:
                    // 螺栓符号
                    g.DrawEllipse(pen, position.X - symbolSize * 0.08f, position.Y - symbolSize * 0.08f,
                                 symbolSize * 0.16f, symbolSize * 0.16f);
                    for (int i = 0; i < 6; i++)
                    {
                        float angle = i * (float)Math.PI / 3;
                        float x1 = position.X + symbolSize * 0.08f * (float)Math.Cos(angle);
                        float y1 = position.Y + symbolSize * 0.08f * (float)Math.Sin(angle);
                        float x2 = position.X + symbolSize * 0.08f * (float)Math.Cos(angle + (float)Math.PI / 3);
                        float y2 = position.Y + symbolSize * 0.08f * (float)Math.Sin(angle + (float)Math.PI / 3);
                        g.DrawLine(pen, x1, y1, x2, y2);
                    }
                    break;
                    
                case AssemblyType.Bonding:
                    // 胶合符号
                    g.DrawEllipse(pen, position.X - symbolSize * 0.06f, position.Y + symbolSize * 0.04f,
                                 symbolSize * 0.12f, symbolSize * 0.12f);
                    g.DrawLine(pen, position.X - symbolSize * 0.2f, position.Y - symbolSize * 0.1f,
                              position.X + symbolSize * 0.2f, position.Y - symbolSize * 0.1f);
                    break;
                    
                default:
                    // 通用装配符号
                    g.DrawLine(pen, position.X - symbolSize * 0.15f, position.Y,
                              position.X + symbolSize * 0.15f, position.Y);
                    g.DrawLine(pen, position.X, position.Y - symbolSize * 0.15f,
                              position.X, position.Y + symbolSize * 0.15f);
                    break;
            }
        }
        ...
        /// 绘制状态指示器
        /// </summary>
        private void DrawStatusIndicator(Graphics g, PointF position, float radius, float scale)
        {
            using (var statusPen = new Pen(GetStatusColor().ToDrawingColor(), 2.0f * scale))
            {
                float indicatorX = position.X + radius * 0.7f;
                float indicatorY = position.Y - radius * 0.7f;
                ...
                switch (Status)
                {
                    case AssemblyStatus.Assembled:
                        // 绿色圆圈
                        g.DrawEllipse(statusPen, indicatorX - indicatorSize * 0.3f, indicatorY - indicatorSize * 0.3f,
                                     indicatorSize * 0.6f, indicatorSize * 0.6f);
                        break;
                    ...
                    case AssemblyStatus.InProgress:
                        ...
                        {
                            new PointF(indicatorX, indicatorY + indicatorSize * 0.3f),
                            new PointF(indicatorX - indicatorSize * 0.26f, indicatorY - indicatorSize * 0.15f),
                            new PointF(indicatorX + indicatorSize * 0.26f, indicatorY - indicatorSize * 0.15f)
                        };
                        g.DrawPolygon(statusPen, triangle);
                        break;
                        
                    case AssemblyStatus.NotAssembled:
                        // 灰色方块
                        g.DrawRectangle(statusPen, indicatorX - indicatorSize * 0.25f, indicatorY - indicatorSize * 0.25f,
