...
{
    /// <summary>
    /// 关联锚点引用: 把标注端点绑定到某宿主实体的捕捉点(角点)索引上。
    /// 宿主几何改变后据此重取端点 — 实现尺寸锚点跟随。会话级, 不持久化。
    /// </summary>
    public sealed class DimAnchor
    {
        public Entity Host { get; }
        public int SnapIndex { get; }
        public DimAnchor(Entity host, int snapIndex) { Host = host; SnapIndex = snapIndex; }
    }
    ...
    public class LinearDimension : DimensionBase
    {
        ...
        /// <summary>
        /// 关联锚点: 端点绑定宿主实体捕捉点(角点)的索引。宿主几何改变后, RefreshFromAnchors 重取端点位置,
        /// 使尺寸锚点跟随透镜变化。会话级 (持有实体引用), 不序列化。
        /// </summary>
        [System.Text.Json.Serialization.JsonIgnore]
        public DimAnchor? FirstAnchor { get; set; }

        /// <summary>第二端点的关联锚点, 见 <see cref="FirstAnchor"/>。</summary>
        [System.Text.Json.Serialization.JsonIgnore]
        public DimAnchor? SecondAnchor { get; set; }

        /// <summary>
        /// 若端点绑定了宿主角点, 从宿主当前捕捉点重取端点位置 (宿主改变 → 尺寸锚点跟随)。
        /// 仅在位置确有变化时重建几何。返回是否发生变化。
        /// </summary>
        public bool RefreshFromAnchors()
        {
            bool ch = false;
            if (TryAnchor(FirstAnchor, out var p1))
            {
                var dx = p1.X - _firstReferencePoint.X; var dy = p1.Y - _firstReferencePoint.Y;
                if (dx * dx + dy * dy > 1e-10) { _firstReferencePoint = p1; ch = true; }
            }
            if (TryAnchor(SecondAnchor, out var p2))
            {
                var dx = p2.X - _secondReferencePoint.X; var dy = p2.Y - _secondReferencePoint.Y;
                if (dx * dx + dy * dy > 1e-10) { _secondReferencePoint = p2; ch = true; }
            }
            if (ch) { UpdateDefinitionPoint(); Update(); }
            return ch;
        }

        private static bool TryAnchor(DimAnchor? a, out Vector2 pt)
        {
            pt = default;
            if (a is null) return false;
            List<ObjectSnapPoint> sps;
            try { sps = a.Host.GetSnapPoints(); } catch { return false; }
            if (sps is null || a.SnapIndex < 0 || a.SnapIndex >= sps.Count) return false;
            pt = sps[a.SnapIndex].position;
            return true;
        }
        ...
        public override void Generate()
        {
            ...
            // 计算文本位置: 文字放在尺寸线"朝被测边"一侧 (-extDir), 即标注线与零件之间 (上方), 隔 gap。
            ...
            // 文本朝向: 竖直标注 (Ø 等) 文字跟随标注线竖排, 不被 TextInsideHorizontal 强制压平。
            // 判定用 cos≈0 (方向竖直), 而非比对 _rotation==±π/2 — 拖动尺寸线越过参考线会 _rotation+=π
            // 翻转 (π/2→3π/2), 旧判定失效就把竖排文字误判成横排。
            bool isVertical = Math.Abs(Math.Cos(_rotation)) < 1e-3;
            // 取向跟随标准/用户格式: 竖直标注默认竖排(GB/ISO, VerticalTextAligned=true);
            // 若标准/格式设 VerticalTextAligned=false, 则竖直标注文字也水平书写。非竖直标注沿用 TextInsideHorizontal。
            bool horizontalText = isVertical ? !_style.VerticalTextAligned : _style.TextInsideHorizontal;

            // 文字旋转规整到可读区间 (-π/2, π/2]: 翻转后 (3π/2 等) 仍保持文字正立, 不上下颠倒。
            double textRot = Math.Atan2(Math.Sin(_rotation), Math.Cos(_rotation));
            if (textRot > Math.PI / 2.0) textRot -= Math.PI;
            else if (textRot <= -Math.PI / 2.0) textRot += Math.PI;
            dimText.Rotation = horizontalText ? 0 : textRot;
            ...
            {
                // 竖排文字: 恒放在尺寸线"左侧"(用户要求统一在左), Y 沿线跟随拖动滑动 → 拖到任一侧都不压线。
                // 渲染器对 π/2 竖排文字: 字列落在锚点 x∈[px-0.8h, px+0.2h] (主要在锚点左侧),
                // 故锚点 X = 线X - gap - 0.2·字高 时, 字列右缘恰在线左 gap 处, 整列在尺寸线左侧。
                dimText.alignment = TextAlignment.CenterBottom;
                double h = _style.TextHeight;
                double anchorX = textAnchor.X - _style.DimensionLineGap - 0.2 * h;
                dimText.Position = new Vector3(anchorX, textAnchor.Y, dimText.Position.Z);
            }
            else
            {
                dimText.alignment = TextAlignment.CenterBottom;
        ...
        public override List<GripPoint> GetGripPoints()
        {
            ...
            // 尺寸线与文字合并为一个夹点 (文字本就贴在尺寸线上): 拖它 — 垂直分量调尺寸线偏移,
            // 平行分量沿尺寸线滑动文字。位置取文字处 (用户直接抓数字)。
        ...
        public override void SetGripPointAt(int index, GripPoint gripPoint, Vector2 newPosition)
        {
            switch (index)
            {
                ...
                case 1: // 第二参考点 — 自由拖
                    ...
                    break;
                case 2: // 合并夹点 (尺寸线+文字): 垂直分量→反推 offset(必要时翻 rotation); 文字随拖点 → 平行分量即沿线滑动
                    SetDimensionLinePosition(newPosition);   // 尺寸线穿过拖点 + settle rotation/_offset
                    // 文字留在拖点, 尺寸线朝远离零件侧外推 DimensionLineGap → 文字落在标注线与零件之间(上方), 隔间距不压线
                    UpdateDimensionLinePosition(_offset + _style.DimensionLineGap);
                    UpdateDefinitionPoint();
