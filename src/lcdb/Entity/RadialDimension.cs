                ...
                case 2: // 引线端 — 拖动绕圆心重新瞄准 (改标注方向, 半径锁定) + 调整引线长度
                    if (_leaderLength > 0)
                    {
                        ...
                        if (r > Utils.EPSILON && (newPosition - _centerPoint).length > Utils.EPSILON)
                        {
                            ...
                            Vector2 direction = (_chordPoint - _centerPoint).normalized;
                            // 钳一个可抓取的最小引线长度: 否则拖到圆心侧会夹到 0, GetGripPoints 不再生成
                            // 引线端夹点 → 夹点消失, 再也拖不回来 (用户报"拖到左边去不了右边")。
                            double minLeader = _style.ArrowSize * 2.0;
                            _leaderLength = Math.Max(minLeader, Vector2.Dot(newPosition - _chordPoint, direction));
